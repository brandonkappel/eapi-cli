    ...
    public partial class CRUDCoordinatorCLIHandler
    {
        public override void AddHelp(StringBuilder sb, string helpTerm)
        {
            ...
            if (helpTerm == "general")
            {
                ...
                sb.AppendLine($"void: GetSchema");
                sb.AppendLine($"void: GetSchemas");
            ...
            if ("getschema".Contains(helpTerm, StringComparison.OrdinalIgnoreCase))
            {
                sb.AppendLine($" - GetSchema");
                if ("getschema".Equals(helpTerm, StringComparison.OrdinalIgnoreCase)) 
                {
                    this.PrintGetSchemaHelp(sb);
                }
                found = true;
            }
            if ("getschemas".Contains(helpTerm, StringComparison.OrdinalIgnoreCase))
            {
                sb.AppendLine($" - GetSchemas");
                if ("getschemas".Equals(helpTerm, StringComparison.OrdinalIgnoreCase)) 
                {
                    this.PrintGetSchemasHelp(sb);
                }
                found = true;
            }
        ...
        private string HandlerFactory(string invokeRequest, string payloadString, string where, Int32 maxPages)
        {
            ...
            switch (invokeRequest.ToLower())
            {
                ...
                case "getschema":
                    this.SMQActor.GetSchema(null, payload, (reply, bdea) =>
                    {
                        result = SerializePayload(reply);
                    }).Wait(30000);
                    break;                   
                ...
                case "getschemas":
                    this.SMQActor.GetSchemas(null, payload, (reply, bdea) =>
                    {
                        result = SerializePayload(reply);
                    }).Wait(30000);
                    break;                   
        ...
        public void PrintGetSchemaHelp(StringBuilder sb)
        {
            
        }
        
        public void PrintGetSchemasHelp(StringBuilder sb)
        {
